...
class QueryBuilder {
  ...
  execute(): string {
    ...
    const orderByPart = this.orderFields.length > 0 ? ` ORDER BY ${this.orderFields.join(', ')}` : '';
    const limitPart = this.limitCount !== undefined ? ` LIMIT ${this.limitCount}` : '';

    return `SELECT ${fieldsPart} FROM ${this.table}${wherePart}${orderByPart}${limitPart};`;
  }
}
